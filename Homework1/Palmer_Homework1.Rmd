...
\newline
\texttt{https://github.com/KarissaPalmer/STAT600/tree/6d81cfc6393aba68d288d120456004d89d0131a5/Homework1/}
\newline
And choose the folder \texttt{SimpLin}. The associated vignette is in the folder \texttt{vignettes}.
...
```{r, datasim, results = 'hide', message = FALSE}
...
```{r, rcpp, echo = FALSE, results = 'hide'}
...
summ_rcpp<- foreach(t = 1:100) %dopar% {
  ...
  mod_rcpp<- SimpLinR(sim_data[[t]][,1], sim_data[[t]][, 2])
  ...
  list(time = tot_time
       , coef = coef_rcpp, ci = ci_rcpp, pred = pred_rcpp)
}
...
```{r, rcode, echo = FALSE, results ='hide'}
...
summ_r<- foreach(t = 1:100) %dopar% {
  ...
  #Put it all in a list
  list(time = tot_time
       , coef = coef_r, ci = ci_r, pred = pred_r)
}
...
When using applying the function \texttt{SimpLinR} and \texttt{lm()} I also used 6 cores each. The function \texttt{SimpLinR} took 0.2370 seconds to go through the simulations and the R function took 0.0957 seconds. However, it is of note that when I commented out the parts where I was getting the necessary data from the functions, \texttt{SimpLinR} was marginally faster than \texttt{lm()}. The file on Github \texttt{Sim\_Data.RData} has the simulated data whose results may be checked.
...
Since the outputs of \texttt{SimpLin} and \texttt{lm()} in terms of their estimated coefficients are the same for each data set, runtime is one of the more telling measurements for differences in approach. We saw in Question 3 that the \texttt{lm()} function was approximately twice as fast when including extracting desired data, but on the individual basis of computation time our package (Rcpp) was about ten times faster than \texttt{lm()} while \texttt{lm()} had slightly more variability in the computation.
...
```{r ref.label="rcpp",eval=FALSE}
```

```{r, ref.label='rcode', eval = FALSE}
```

```{r, ref.label = 'msd', eval = FALSE}
```

```{r, ref.label = 'sum_stat', eval = FALSE}
```

```{r, ref.label = 'hists', eval = FALSE}
```